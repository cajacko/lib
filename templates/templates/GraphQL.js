...
class GraphQL extends StartTemplate {
  constructor(...args) {
    super(...args);

    this.setTemplateSrc(join(__dirname, 'files/graphql/src'));
    this.setSrcDestination('functions/src/projectFiles');
  ...
  copy() {
    ...
    return Promise.all([
      this.fs.copyTmpl(
        join(__dirname, 'files/graphql/graphqlConfig.js'),
        join(this.tmpDir, 'functions/src/config.js'),
        { entryPath }
      ),
